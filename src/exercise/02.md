...
- Memoization: avoid recomputing the same thing over and over and return the same output if the inputs are the same
- React facilitates memoization in 3 ways:
...
```
- A new function is created on every render. React will return the same function as the last render if the deps have not changed

2. useMemo
- It is the more generic hook that can be used for memoizing any value, not just a function.
- In `useMemo`, the value returned by the function is memoized and not the function itself.
- In fact, `useCallback` is just syntactic sugar on top of `useMemo`
...
3. memo
- Specific memoization to react components
- Typically shouldn't be used unless we can quantify performance gains
- Used for functional components only
- For class based components, prefer to extend `PureComponent`
```javascript
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=02%3A%20useCallback%3A%20custom%20hooks&em=deborah.gertrude.digges%40gmail.com).
