...
- When is `useReducer` helpful:
  - When we want to separate the state logic from the component using that state
  - When we have an multiple elements of state that typically change together
  - When we have a single element of state to maintain, it is typically better to use `useState`
- The API is: `const [state, dispatch] = useReducer(reducerFunction, initialState)`
  - API for reducer function: `reducerFunction(currentState, action)`
    - Here `action` is whatever argument is passed to `dispatch`. It is typically an object, but could also be a single value
    - `action` could also be a function in which case the dispatch function will be called like `dispatch(previousState => { // new object constructed from prev state})`
    - In this case, the reducer function would test if `action` is a function, and if it is will call `action` with `previousState`
    ```
      function reducer(previousState, action) {
        return typeof(action) === 'function'? action(previousState): action.value
      }
    ```
- `useReducer` can also be used with a lazy initializer: `useReducer(reducerFunction, initValue, lazyInitFunction)`
  - In this case, `initValue` is passed as an argument to `lazyInitFunction` and the return value of `lazyInitFunction` is used to initialize the reducer
  - Lazy initialization would be used if the initialization operation was expensive and involved, say, reading from local storage or making a network call.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=01%3A%20useReducer%3A%20simple%20Counter&em=deborah.gertrude.digges%40gmail.com).
